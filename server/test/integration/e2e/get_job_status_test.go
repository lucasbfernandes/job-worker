...
type GetJobStatusE2EIntegrationTestSuite struct {
	suite.Suite
...
func (suite *GetJobStatusE2EIntegrationTestSuite) SetupSuite() {
...
func (suite *GetJobStatusE2EIntegrationTestSuite) TearDownSuite() {
...
func (suite *GetJobStatusE2EIntegrationTestSuite) TestShouldReturn401WhenAuthHeaderIsNotPresent() {
	...
	resp, err := http.Get(
		fmt.Sprintf("%s/jobs/"+job.ID+"/status", suite.tlsServer.URL),
	)
	if err == nil {
		_ = resp.Body.Close()
	}
...
func (suite *GetJobStatusE2EIntegrationTestSuite) TestShouldReturn401WhenAuthHeaderExistsButIsInvalid() {
	job := jobEntity.NewJob([]string{"ls", "-la"}, suite.admin.ID)
	err := suite.server.Interactor.Database.UpsertJob(job)
	assert.Nil(suite.T(), err, "upsert job returned with error")
...
func (suite *GetJobStatusE2EIntegrationTestSuite) TestShouldReturn200WhenAuthHeaderExistsAndIsCorrect() {
	...
	req, err := http.NewRequest(
		"GET",
		fmt.Sprintf("%s/jobs/"+response.ID+"/status", suite.tlsServer.URL),
		nil,
	)
	assert.Nil(suite.T(), err, "failed to create http request")
...
func TestGetJobStatusE2EIntegrationTest(t *testing.T) {
	suite.Run(t, new(GetJobStatusE2EIntegrationTestSuite))
}
