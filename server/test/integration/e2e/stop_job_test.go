...
type StopJobE2EIntegrationTestSuite struct {
	suite.Suite
...
func (suite *StopJobE2EIntegrationTestSuite) SetupSuite() {
...
func (suite *StopJobE2EIntegrationTestSuite) TearDownSuite() {
...
func (suite *StopJobE2EIntegrationTestSuite) TestShouldReturn401WhenAuthHeaderIsNotPresent() {
	...
	resp, err := http.Post(
		fmt.Sprintf("%s/jobs/"+job.ID+"/stop", suite.tlsServer.URL),
		"application/json",
		nil,
	)
	if err == nil {
...
func (suite *StopJobE2EIntegrationTestSuite) TestShouldReturn401WhenAuthHeaderExistsButIsInvalid() {
	job := jobEntity.NewJob([]string{"ls", "-la"}, suite.admin.ID)
	err := suite.server.Interactor.Database.UpsertJob(job)
	assert.Nil(suite.T(), err, "upsert job returned with error")
...
func (suite *StopJobE2EIntegrationTestSuite) TestShouldReturn401WhenAuthHeaderExistsIsCorrectButFromDiffUser() {
	job := jobEntity.NewJob([]string{"ls", "-la"}, suite.admin.ID)
	err := suite.server.Interactor.Database.UpsertJob(job)
	assert.Nil(suite.T(), err, "upsert job returned with error")
...
func (suite *StopJobE2EIntegrationTestSuite) TestShouldReturn200WhenAuthHeaderExistsAndIsCorrect() {
	...
	req, err := http.NewRequest(
		"POST",
		fmt.Sprintf("%s/jobs/"+response.ID+"/stop", suite.tlsServer.URL),
		nil,
	)
	assert.Nil(suite.T(), err, "failed to create http request")
...
func TestStopJobE2EIntegrationTest(t *testing.T) {
	suite.Run(t, new(StopJobE2EIntegrationTestSuite))
}
